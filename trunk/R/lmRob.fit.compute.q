			      ...
			      robust.control=NULL, genetic.control=NULL, ...)
{
    ...
    if(init.alg == "auto")
	init.alg <-
	    if((n <= 250 && p == 2) || (n <= 80 && p == 3)) "exhaustive"
	    else if (p2 > 15) "fast"
	    else "random"
    ...
    switch(init.alg,
	   ...
	   "exhaustive" = {
		   ...
		   stop(gettextf(
		  "initial algorithm 'exhaustive' needs too many (%d) repetitions",
				 nrep))
	       if (n > 300 || p2 > 10)
		   ## was an error [stop()], but the user should be allowed to shoot himself
		   warning(gettextf(paste(
		  "The data set is relatively large, and \"exhaustive\" needs %d subsamples",
	   ...
	   "fast" = iop <- 6,
	   ## otherwise :
	   stop(gettextf("Invalid choice of resampling method: init.alg = %s",
			 init.alg))
	   )# end{switch}
    ...
    if(iop == 5) { ## <==> p2 == 0: *all* predictors are categorical
	## ==> M-estimate
    ...
    else if(iop == 4) {	 ## -- "genetic"

        stopifnot("'genetic' is no longer available")
	##    set.seed(seed)
	##
    ...
    else if(!is.null(x1)) { ##-- continuous + factor-variables
	## ===>	  use "alternate M-S"
	...
	z1 <- .Fortran("rlfrstml", #--> ../src/lmrobml.f  -> ../src/lmrobmm.f
		       X1 = x1,
		       X2 = x2.tilde,
		       Y = as.double(y.tilde),
		       as.integer(n),
	...
	if (final.alg == "mm" || final.alg == "m") {
		...
		if(intercept) {
		    ...
		    lmRob.wm(tmp, y, ymt, ucov0[1], scale0, isigma=0,
